"""A collecton of functions written either for study group
for the 02221FT cohort or by James Irving. 

Source study group file:
"""
...
def multiplot(df_model,figsize=(12,12),cmap="Reds",drop=None,use_mask=True):
    if drop is not None:
        corr = df_model.drop(columns=drop).corr()
    else:
        corr = df_model.corr()
    ...
    if use_mask==False:
        mask=None
    sns.heatmap(corr, annot=True,cmap=cmap,mask=mask,square=True,cbar_kws={'shrink': 0.5},ax=ax)
    return fig, ax